...
def ssxx():
    ...
    for i in range(len(JSON['data']['content_info'])):
        if (len(JSON['data']['content_info'][i]['section_list'])):
            print("正在观看----" + JSON['data']['c_short_name'] + " 第" + str(i+1) + "章" + "----共找到" + str(
                len(JSON['data']['content_info'][i]['section_list'])) + "个视频。")
            for j in range(len(JSON['data']['content_info'][i]['section_list'])):
                # print(len(JSON['data']['content_info'][i]['section_list']))
                for leaf_list in range(len(JSON['data']['content_info'][i]['section_list'][j]['leaf_list'])):
                    cards_id = '0'
                    if JSON['data']['content_info'][i]['section_list'][j]['leaf_list'][leaf_list]['leaf_type'] == 0:
                        # 0改成1
                        video_id = str(JSON['data']['content_info'][i]['section_list'][j]['leaf_list'][leaf_list]['id'])
                        # print(video_id)
            ...
            for i in range(len(JSON['data']['content_info'])):
                print("正在观看----" + JSON['data']['c_short_name'] + " 第" + str(i+1) + "章" + "----共找到" + str(
                    len(JSON['data']['content_info'][i]['leaf_list'])) + "个视频。")
                for j in range(len(JSON['data']['content_info'][i]['leaf_list'])):
                    # print(len(JSON['data']['content_info'][i]['leaf_list']))
                    cards_id = '0'
                    if JSON['data']['content_info'][i]['leaf_list'][j]['leaf_type'] == 0:
                        # 0改成1
                        video_id = str(
                            JSON['data']['content_info'][i]['leaf_list'][j]['id'])
                        print(video_id)
                        url = 'https://www.yuketang.cn/mooc-api/v1/lms/learn/leaf_info/' + classroom_id + '/' + video_id + '/'
                        response = session.get(url=url, headers=headers)
                        ...
                        while sunci != 1:
                            for k in range(25):
                                time.sleep(0.5)
                                print("正在观看第" + str(i) + "章 第" + str(j + 1) + "个视频----当前进度：" + str(
                                    4 * (k + 1)) + "%")
                                url = 'https://www.yuketang.cn/video-log/heartbeat/'
                                data = '{"heart_data":[{"i":5,"et":"heartbeat","p":"web","n":"ali-cdn.xuetangx.com","lob":"ykt","cp":' + str(
                                    d * (1 + k) / 25) + ',"fp":100,"tp":100,"sp":5,"ts":"' + str(
                                    timestamp_ms + d * (
                                            1 + k) * 2500) + '","u":' + u + ',"uip":"","c":' + c_course_id + ',"v":' + v + ',"skuid":' + str(
                                    s_id) + ',"classroomid":"' + classroom_id + '","cc":"' + ccid + '","d":' + str(
                                    d) + ',"pg":"' + video_id + '_x33v","sq":11,"t":"video","cards_id":0,"slide":0,"v_url":""}]}'
                                ...
                                if sunci == 1:
                                    break
    print("这门课看完了啊！ R_0Ce@N期待与您的下次相遇！")
